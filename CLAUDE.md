...
4. **タスク分割実装** — 各タスク = 実装 → 仕様レビュー(実装・テストが仕様どおりかの確認)。
   指摘を反映してから次タスクへ進む。
   **前倒しレビューの例外** — 次に該当するタスクはタスク時に追加レビューを行う
   (該当判定に迷ったら前倒しに倒す):
   - セキュリティ敏感面(外部入力のパース・パス操作・プロセス起動・WebView/プレビュー・
     ネットワーク)に触れる → **脆弱性レビュー**を実施
   - 後続タスクが依存する新しい抽象・seam・共通パターンを導入する → **コード品質レビュー**を実施
5. **最終ブランチレビュー(2 パス)** — ブランチ全体を**コード品質パス**
   (ミューテーション検証のスポットチェック込み)と**脆弱性パス**の焦点別 2 パスでレビューし、
   指摘は fixup commit で反映。パスごとに独立した別エージェントを起動する
   (1 起動に混載するとレビューが浅くなるため)。
6. **品質ゲート(§6)→ PR(§7)**。

superpowers プラグイン導入環境では各工程を対応スキルで実施する(推奨・必須ではない。
スキル既定の手順(レビュー段数等)と本書が食い違う場合は本書 §3 が優先):
...
最終レビューの 2 パスも 1 回に統合してよい。ただし別エージェントレビューと品質ゲートは省略しない。

## 4. レビュー標準

- マージ前に**別エージェントによるレビューを必ず**行う(§3 の工程に乗らない変更=
  docs や CI 設定のみの変更も同様)。
- 指摘は鵜呑みにしない。技術的に検証し、妥当でない指摘は理由付きで却下してよい。
- 最終ブランチレビューのコード品質パスでは**ミューテーション検証**をスポットチェックで実施する:
  高価値テスト(Cancel / Guard 系・境界系など)に絞り、実装行を一時的に変異させ、
  対象テストが赤になることを確認してから復元する。
...
- main へのマージ前に `tools/pre-merge-check.ps1` を実行し **EXIT 0** を確認する(CI と同種のゲート。
  ...
  **例外**: ブランチ差分がドキュメントのみ(ビルド・テストに影響しない)の場合は省略してよい
  (§4 の別エージェントレビューは省略しない)。

...
```
Expected: `OK: pre-merge チェック全通過`(Release 0 警告・Core 570+Editor 216+App 14=**800** 緑)。
...
- Task 1: `8079482`(App 層摘出・7 files・+11/−140)。仕様準拠レビュー✅・品質レビュー承認(Critical/Important 0・Minor 4=コメント衛生2件を Task 2 同乗・Raise 整理を Stage 2 申し送り・grep 拡張を Task 2 検証に反映)
- Task 2: `a17c920`(イベント土台削除・13 files・+49/−642)+`1a946e3`(レビュー指摘対応: 孤児化 TestHook_SendKey 削除)。仕様準拠レビュー✅・品質レビュー承認(Critical/Important 0)
- 逸脱(実測補正): EmptyLineDetectorTests は 18 ケース(計画の 19 は過大見積り)→テスト数の正は 831→**800**(Core 570・Editor 216・App 14)。本記録の Task 2 Step 9 期待値 569/799 は 570/800 に読み替え
- 残存する PC-Talker 言及(意図的温存): 温存指定の歴史コメント(TextRangeProviderV2×3・EditorControl フォーカス発火・GrepResultsWindow・RestoreDialog・NavigationCommands)+廃止経緯の説明(AnnouncerFactory)+移設テストの出自コメント(RaiseUiaSelectionEventsTests)+温存実装と対の TextRangeProviderV2Tests:125

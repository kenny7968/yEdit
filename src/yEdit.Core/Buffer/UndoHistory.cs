...
internal sealed class UndoHistory
{
    internal readonly record struct Entry(
        PieceTree.Node? RootBefore, PieceTree.Node? RootAfter, int Pos, int RemovedLen, int InsertedLen);
    ...
    public void Record(PieceTree.Node? before, PieceTree.Node? after,
                       int pos, int removedLen, int insertedLen, bool insertHasBreak)
    {
        ...
        bool coalescable = (pureInsert && insertedLen <= 2 && !insertHasBreak)
                        || (pureDelete && removedLen <= 2);

        if (_open && coalescable && _undo.Count > 0)
        {
            var prev = _undo[^1];
            if (pureInsert && prev.RemovedLen == 0 && prev.InsertedLen > 0
                && pos == prev.Pos + prev.InsertedLen)
            {   // タイプ継続(RootBefore は据え置き)
                _undo[^1] = prev with { RootAfter = after, InsertedLen = prev.InsertedLen + insertedLen };
                return;
            }
            if (pureDelete && prev.InsertedLen == 0 && prev.RemovedLen > 0)
            {
                ...
                {   // Backspace(逆方向)
                    _undo[^1] = prev with { RootAfter = after, Pos = pos, RemovedLen = prev.RemovedLen + removedLen };
                    return;
                }
                if (pos == prev.Pos)
                {   // Delete前方
                    _undo[^1] = prev with { RootAfter = after, RemovedLen = prev.RemovedLen + removedLen };
                    return;
                }
            }
        }

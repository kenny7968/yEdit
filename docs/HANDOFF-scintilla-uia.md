...
- **決着（2026-07-03・能動発声方式）**: UIA 側での解決（偽テキスト注入）は不可のため、**純ナビゲーション（本文変更なし・選択なし）で行が変わり空行へ着地したときだけ、App 層が Announcer 経由で「空行」を能動発声**する方式で解消。
  ...
  - **申し送り（v1 許容・実機で要観察）**: (a) Ctrl+G で空行へジャンプ時「行 N」と「空行」が両方割り込み priority=1 で発声され後着が勝つ（下記の遅延化により通常は「空行」が後着＝勝ち）。(b) タブ切替/フォーカス復帰で空行上に居る場合は行変化ゲートに掛からず従来どおり無音。(c) CSV モード終了時の遅延 SCN_UPDATEUI は両ガードをすり抜け得るが、CSV パーサが空行から行を生成しないため現行実装では実害なし（パーサ仕様への暗黙依存）。(d) 実機検証項目: 空行着地の「空行」可聴／PC-Talker の連続読みを空行通過ごとの割り込みが中断しないか／空白のみ行の受動読みが無音でないか。
- **追記（2026-07-03・遅延発声化）**: 初回実機検証で**「空行」が無音**だった。他の能動アナウンス（同じ `PCTKPReadW(text,1,1)` 経路）は可聴のため、原因は **PC-Talker 自身のキャレット行読みとの競合**が最有力: 着地時に yEdit が UIA 選択変更イベント発火→直後に「空行」を発声するが、PC-Talker がイベントを受けて着地行（空行=無音）を読む際の**発話フラッシュ**が数十 ms 遅れて届き、「空行」を言い終わる前に消す（上記 (a) の「後着勝ち」と同じ機構で、空行は相手が無音のため「無音が勝つ」）。対策: **MainForm で発声を一発タイマー 150ms（`_emptyLineDelay`）で遅延**し、満了時に条件を再検証（アクティブ・非 CSV・エディタにフォーカス・選択なし・今も空行上）してから `Say("空行")`。Stop→Start 再始動により連続空行移動はデバウンス（最後の着地だけ読む）。**実機で依然無音/途切れる場合は Interval を 250〜300ms へ引き上げて再検証**。それでも無音なら底部通知ラベルに「空行」が出るかで切り分け（出る=発声側→`PcTalkerSpeech.Speak` の差し替え、出ない=検知側）。

...
public static class VerticalNavigation
{
    ...
    private static (int caret, int desiredPx) MoveVerticalRelative(
        TextSnapshot snap, int caret, int currentDesiredPx, int wrapColumns, ICharMetrics metrics, int deltaRows)
    {
        ...
        if (wrapColumns > 0)
        {
            (targetLogicalLine, targetSegIdx) = WalkVisualRows(
                snap, logicalLine, segIdx, segs.Count, deltaRows, maxWidthPx, metrics);
        }
        else
        {
        ...
        string targetLineText = targetLineEnd == targetLineStart
            ? string.Empty
    ...
    private static (int line, int seg) WalkVisualRows(
        TextSnapshot snap, int startLine, int startSeg, int startSegCount,
        int deltaRows, int maxWidthPx, ICharMetrics metrics)
    {
        ...
        while (remain > 0)
        {
            if (step > 0)
            {
                ...
                {
                    if (line + 1 >= snap.LineCount) return (line, seg); // 文書末で打ち切り
                    line++;
                    seg = 0;
            ...
            else
            {
                ...
                {
                    if (line == 0) return (line, seg); // 文書頭で打ち切り
                    line--;
                    count = SegmentCount(snap, line, maxWidthPx, metrics);
                    seg = count - 1;
                }
    ...
    private static int SegmentCount(TextSnapshot snap, int line, int maxWidthPx, ICharMetrics metrics)
    {

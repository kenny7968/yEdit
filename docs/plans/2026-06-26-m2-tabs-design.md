...
> **実装時の改訂（実機SR検証 2026-06-26）**: Q3 の「切替後は編集領域へフォーカス→現在行を読む」を改訂。
> NVDA は「フォーカスした要素」しか読まないため、切替直後にエディタへフォーカスするとファイル名ではなく
> エディタのロール（複数行エディット）を読んでしまう（MSAA に名前を載せる案も実機で効果なし＝取消し、
> `ScintillaHost` は無改変に復帰）。確定挙動: **`Ctrl+Tab`/`Ctrl+数字`/`←→` での切替後はフォーカスをタブ列に
> 留める**（標準 TabControl の読み上げで「ファイル名＋位置」を NVDA・PC-Talker とも読む）→ **`Enter`（または `Tab`）で
> エディタへ入って編集**。新規/開く/タブを閉じた直後は従来どおりエディタへ直接フォーカス。下記 §3 もこの挙動に従う。

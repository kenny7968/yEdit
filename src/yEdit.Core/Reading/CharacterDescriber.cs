...
public static class CharacterDescriber
{
    ...
    public static string DescribeAt(string text, int index)
    {
        ...
        int cp = char.ConvertToUtf32(text, index);
        return Describe(cp);
    }

    /// <summary>コードポイントを日本語で説明する。</summary>
    public static string Describe(int codePoint)
    {
        ...
        // それ以外（記号・各種文字・サロゲート上の文字）は文字＋コードポイント。
        return codePoint <= 0xFFFF ? $"{s} (U+{codePoint:X4})" : $"{s} (U+{codePoint:X5})";
    }
    ...
    private static bool IsCjkIdeograph(int cp) =>
        cp is (>= 0x4E00 and <= 0x9FFF)   // CJK 統合漢字
            ...
            or (>= 0x20000 and <= 0x2FA1F); // 拡張B〜（astral）
}

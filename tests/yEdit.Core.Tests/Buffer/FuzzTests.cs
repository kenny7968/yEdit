...
public class FuzzTests
{
    ...
    [
        "word", "hello world ", "test123", "ひらがな漢字とカナ", "😀🈴", "😀",
        "\n", "\r", "\r\n", "line1\nline2\r\nline3\r", "あiうeお😀\r\n混合テキスト", "x",
    ];
    ...
    {
        int ops = int.TryParse(Environment.GetEnvironmentVariable("YEDIT_FUZZ_OPS"), out int v) && v > 0
            ? v : 3000;
        ...
        for (int op = 1; op <= ops; op++)
        {
            ...
            if (text.Length > MaxDocLength) roll = 45;   // Delete優先
            ...
            if (op % 25 == 0) DeepVerify(text, buffer.Current, rnd);
        }
    ...
    private static string ModelSplice(string text, int pos, int delLen, string ins,
                                      List<string> undoStack, List<string> redoStack)
    {
        ...
        int end = endRaw > 0 && endRaw < text.Length && char.IsLowSurrogate(text[endRaw]) ? endRaw - 1 : endRaw;
        if (end < start) end = start;
        if (start == end && ins.Length == 0) return text;   // 無変化はUndoエントリなし

        undoStack.Add(text);
        if (undoStack.Count > ModelUndoCap) undoStack.RemoveAt(0);
        redoStack.Clear();
    ...
    private static string Material(Random rnd)
    {
        ...
        while (sb.Length < target) sb.Append(Pool[rnd.Next(Pool.Length)]);
        return sb.ToString();
    }
    ...
    private static void DeepVerify(string text, TextSnapshot snap, Random rnd)
    {
        ...
        Assert.Equal(isLast ? text.Length : ends[line] + 1, snap.GetLineEnd(line, includeBreak: true));
        Assert.Equal(isLast
                ? text.Length
                : ends[line] - (text[ends[line]] == '\n' && ends[line] > 0 && text[ends[line] - 1] == '\r' ? 1 : 0),
            snap.GetLineEnd(line, includeBreak: false));

        int pos = rnd.Next(text.Length + 1);
        Assert.Equal(ends.Count(e => e < pos), snap.GetLineIndexOfChar(pos));

        int a = rnd.Next(text.Length + 1), b = rnd.Next(text.Length + 1);
        if (a > b) (a, b) = (b, a);

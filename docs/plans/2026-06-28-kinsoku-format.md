...
public void LineStart_kinsoku_pushes_forbidden_char_down()
    // columns=4(全角2)で "あい" の後 "。" が行頭に来るのを避け、"い" ごと次行へ追い出す
    => Assert.Equal("あ\nい。う", Start("あい。う", 4, "。"));
...
> 期待値の根拠: 1つ目は cut が「。」の前(=次行先頭が「。」)になるため追い出して「い」を次行へ。2つ目は戻そうとした直前(cut-1)が「。」で連鎖防止に当たり処理しない。
...
public void LineEnd_kinsoku_pushes_opening_bracket_down()
    // columns=4 で現在行末尾が開き括弧「（」になるのを避け、次行へ送る
    => Assert.Equal("あ\nい（う", KinsokuFormatter.Format("あい（う", 4, "", "（", "", "\n"));
